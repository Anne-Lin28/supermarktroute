/***********************
 * Supermarkt Route App (autosuggest alleen tijdens typen)
...
function ensureSuggestBox() {
  ...
  // Basis styling zodat het er netjes uitziet, maar niet “dominant”
...
function getKnownProducts() {
  // Combineer geleerde producten + huidige lijst (voor wat extra hits)
...
function renderSuggestionsForInputValue() {
  const input = getInputEl();
  if (!input || !suggestBoxEl) return;

  const q = normalizeProductNaam(input.value);

  // ✅ BELANGRIJK: alleen tonen als je echt iets getypt hebt
  if (!q) {
    clearSuggestions();
    return;
  }
  ...
  // Filter (contains-match, zodat “melk” ook gevonden wordt als je “elk” typt)
  const matches = all
    .filter(p => p.includes(q))
    .sort((a, b) => a.localeCompare(b, "nl"))
    .slice(0, 5); // max 5 suggesties

  // Niks gevonden → niets tonen (geen tekst die je scherm inneemt)
  if (matches.length === 0) {
    clearSuggestions();
    return;
  }
...
function setupAutosuggest() {
  ...
  input.addEventListener("input", renderSuggestionsForInputValue);

  // Als je het veld verlaat: suggesties weg
...
window.wisGeleerdeProducten = function () {
